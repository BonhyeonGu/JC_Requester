...
UPDATE_ROUTE = [
    "/debugUpdate/air_temperature", "/debugUpdate/pm10", "/debugUpdate/pm25", "/debugUpdate/humidity", "/debugUpdate/voc", "/debugUpdate/noise", "/debugUpdate/iluminance",
    "/debugUpdate/visitor", "/debugUpdate/revisitor", "/debugUpdate/traffic_volume"
]
...
def makeList(type: str) -> list:
    ...
    if type == 'emmUpdate':
        ...
        random.shuffle(RDF_EDIT_LIST_0)
        for i in RDF_EDIT_LIST_0:
            ret.append(f"CMD::mv {os.path.join(LOCALE_BACK, i)} {os.path.join(LOCALE_RDF, i)}")
            ret.append("RESTART")
            REAL_TEST_ROUTE = []
